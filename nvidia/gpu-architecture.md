...
*  TF32 텐서 코어는 FP32 데이터를 읽고, 내부적으로 정밀도를 줄인 상태로 연산한 후, 표준 IEEE FP32 형식의 출력값을 생성합니다.
![image](https://developer.nvidia.com/blog/wp-content/uploads/2020/05/TensorFloat32-TF32.jpg)

| 항목                   | FP32 (Float32)         | TF32 (TensorFloat32)             | FP16 (Float16)               |
|------------------------|------------------------|----------------------------------|------------------------------|
| **총 비트 수**        | 32비트                 | 32비트                           | 16비트                       |
| **지수부 (Exponent)** | 8비트                  | 8비트                            | 5비트                        |
| **가수부 (Mantissa)** | 23비트                 | 10비트 *(FP16과 동일)*           | 10비트                       |
| **부호비트**           | 1비트                  | 1비트                            | 1비트                        |
| **표현 가능한 수 범위** | 매우 넓음              | **FP32와 동일한 범위**           | 좁음                         |
| **정밀도**            | 높음                   | 중간 *(FP16 수준 정밀도)*        | 낮음                         |
| **용도**               | 정밀한 계산, 과학/HPC   | **딥러닝 학습 최적화 (Ampere)** | 딥러닝 학습 및 추론          |
| **텐서 코어 지원**     | 없음 *(기본 연산 경로)* | **지원 (Ampere부터 기본 적용)** | **지원**                     |
| **성능 (A100 기준)**  | 기준선                 | **최대 10배 빠름 (V100 FP32 대비)** | **최대 20배 빠름 (sparisty 포함)** |


### 메모리 및 캐시
* 공유 메모리 + L1 캐시 통합 용량은 192KB로 V100 대비 1.5배이다.
* 비동기 복사 명령어: 글로벌 메모리에서 공유 메모리로 직접 데이터를 로드하며, L1 캐시를 우회할 수 있고, 중간 레지스터 파일(RF) 사용을 제거한다.
* 비동기 barrier 유닛: 비동기 복사 명령어와 함께 사용 가능하다.
* L2 캐시 관리 및 레지던시 제어를 위한 새로운 명령어을 도입하였다.
* CUDA Cooperative Groups를 위한 warp-level reduction 명령어를 추가하였다.
* 소프트웨어 복잡도를 줄이기 위한 다양한 프로그래밍 기능을 개선하였다.

### AMP(Automatic Mixed Precsion)
* AMP을 사용할 경우, NVIDIA GPU 아키텍처에서는 여러 **포맷 조합(FP32, FP16, TF32, BF16)**이 자동으로 선택되며, 성능과 정밀도 간의 균형을 맞추게 된다.
* 아래 표는 AMP 사용 시 포맷 조합과 A100 기준 성능 차이를 보여준다.

| AMP 조합 구분            | 주요 연산 포맷          | 사용 사례                          | 성능 (V100 대비)      | 설명 |
|--------------------------|--------------------------|-------------------------------------|------------------------|------|
| **기본 (미사용)**        | FP32-only                | 고정밀 수치 계산, 초기 모델 구현   | 1x                     | 텐서 코어 미사용, 가장 느림 |
| **TF32 (기본 AMP)**      | TF32 + FP32 출력         | 딥러닝 학습 기본값 (Ampere 이상)   | 최대 10x               | 텐서 코어 활용, 정밀도는 FP16 수준, 출력은 FP32 |
| **AMP with FP16**        | FP16 + FP32 출력         | 고속 학습, 정밀도 일부 손실 허용   | 최대 20x (sparsity 포함) | 대부분의 가중치와 연산을 FP16 처리, 출력은 FP32 |
| **AMP with BF16**        | BF16 + FP32 출력         | Intel/NVIDIA 협업, TPU 호환성      | FP16과 유사            | 학습 안정성이 FP16보다 높고 TF32보다 빠름 |
| **AMP with Sparsity**    | FP16 or INT8 + sparsity  | 프루닝된 모델 추론                | 최대 40x 이상          | A100, H100에서 구조적 희소성(Sparse Tensor Core) 활용 |
